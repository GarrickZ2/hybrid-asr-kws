...
vocab=$lm_dir/voc.txt
# python local/convert_word2voc.py $lm_dir/words.txt > $lm_dir/voc.txt
...
if [ $stage -le 4 ]; then
  (echo '!SIL SIL'; echo '<SPOKEN_NOISE> SPN'; echo '<UNK> SPN'; echo 'PSI1 SIL'; echo '<PHNSILSP> SIL'; \
  cat $nonsil_phones | awk '{for(i = 1; i <= NF; i++) { print "PHN_"$1" "$i;}}' ) | \
  cat - $lexicon_raw_nosil | sort | uniq >$dst_dir/lexicon_w_oovs.txt
  echo "Finish write lexicon_w_oovs.txt"
  awk 'NR==FNR{a[$1];next}!($1 in a){print;}' conf/oovkwlist.txt $dst_dir/lexicon_w_oovs.txt > $dst_dir/lexicon.txt
