...
wrddir=$4
phndir=$5
mkdir -p $dir $tmpdir $wrddir/phones $phndir
...
mkdir -p $wrddir/phones  # various sets of phones...
...
if $share_silence_phones; then
  ...
  cat $srcdir/silence_phones.txt | awk '{printf("%s ", $0); } END{printf("\n");}' | cat - $srcdir/nonsilence_phones.txt | \
    utils/apply_map.pl $tmpdir/phone_map.txt > $wrddir/phones/sets.txt
  cat $wrddir/phones/sets.txt | \
    awk '{if(NR==1) print "not-shared", "not-split", $0; else print "shared", "split", $0;}' > $wrddir/phones/roots.txt
else
  ...
  cat $srcdir/{,non}silence_phones.txt | utils/apply_map.pl $tmpdir/phone_map.txt > $wrddir/phones/sets.txt
  cat $wrddir/phones/sets.txt | awk '{print "shared", "split", $0;}' > $wrddir/phones/roots.txt
fi
cat $srcdir/silence_phones.txt | utils/apply_map.pl $tmpdir/phone_map.txt | \
  awk '{for(n=1;n<=NF;n++) print $n;}' > $wrddir/phones/silence.txt
cat $srcdir/nonsilence_phones.txt | utils/apply_map.pl $tmpdir/phone_map.txt | \
  awk '{for(n=1;n<=NF;n++) print $n;}' > $wrddir/phones/nonsilence.txt
cp $srcdir/optional_silence.txt $wrddir/phones/optional_silence.txt
cp $wrddir/phones/silence.txt $wrddir/phones/context_indep.txt
# if extra_questions.txt is empty, it's OK.
cat $srcdir/extra_questions.txt 2>/dev/null | utils/apply_map.pl $tmpdir/phone_map.txt \
  >$wrddir/phones/extra_questions.txt
...
if $position_dependent_phones; then
  for suffix in _B _E _I _S; do
    (set -f; for x in `cat $srcdir/nonsilence_phones.txt`; do echo -n "$x$suffix "; done; echo) >>$wrddir/phones/extra_questions.txt
  done
  for suffix in "" _B _E _I _S; do
    (set -f; for x in `cat $srcdir/silence_phones.txt`; do echo -n "$x$suffix "; done; echo) >>$wrddir/phones/extra_questions.txt
  done
fi
...
( for n in `seq 0 $ndisambig`; do echo '#'$n; done ) >$wrddir/phones/disambig.txt
# Create phone symbol table.
if [[ ! -z $phone_symbol_table ]]; then
  start_symbol=`grep \#0 $phone_symbol_table | awk '{print $2}'`
  echo "<eps>" | cat - $wrddir/phones/{silence,nonsilence}.txt | awk -v f=$phone_symbol_table '
  BEGIN { while ((getline < f) > 0) { phones[$1] = $2; }} { print $1" "phones[$1]; }' | sort -k2 -g |\
    cat - <(cat $wrddir/phones/disambig.txt | awk -v x=$start_symbol '{n=x+NR-1; print $1, n;}') > $wrddir/phones.txt
else
  echo "<eps>" | cat - $wrddir/phones/{silence,nonsilence,disambig}.txt | \
    awk '{n=NR-1; print $1, n;}' > $wrddir/phones.txt
fi
# Create a file that describes the word-boundary information for
# each phone.  5 categories.
if $position_dependent_phones; then
  cat $wrddir/phones/{silence,nonsilence}.txt | \
    awk '/_I$/{print $1, "internal"; next;} /_B$/{print $1, "begin"; next; }
         /_S$/{print $1, "singleton"; next;} /_E$/{print $1, "end"; next; }
         {print $1, "nonword";} ' > $wrddir/phones/word_boundary.txt
else
  # word_boundary.txt might have been generated by another source
  [ -f $srcdir/word_boundary.txt ] && cp $srcdir/word_boundary.txt $wrddir/phones/word_boundary.txt
fi
...
cat $tmpdir/lexiconp.txt | awk '{print $1}' | sort | uniq  | awk '
  ...
  END {
    ...
    printf("<s> %d\n", NR+2);
    printf("</s> %d\n", NR+3);
  }' > $wrddir/words.txt || exit 1;
# format of $wrddir/words.txt:
#<eps> 0
#!EXCLAMATION-POINT 1
#!SIL 2
#"CLOSE-QUOTE 3
...
[ -z "$silphone" ] && \
  ( echo "You have no optional-silence phone; it is required in the current scripts"
   ...
   exit 1;
# create $wrddir/phones/align_lexicon.{txt,int}.
...
cat $tmpdir/align_lexicon.txt | \
 perl -ane '@A = split; print $A[0], " ", join(" ", @A), "\n";' | sort | uniq > $wrddir/phones/align_lexicon.txt
# create phones/align_lexicon.int
cat $wrddir/phones/align_lexicon.txt | utils/sym2int.pl -f 3- $wrddir/phones.txt | \
  utils/sym2int.pl -f 1-2 $wrddir/words.txt > $wrddir/phones/align_lexicon.int
...
if $silprob; then
  ...
  utils/make_lexicon_fst_silprob.pl $tmpdir/lexiconp_silprob_disambig.txt $srcdir/silprob.txt $silphone '#'$ndisambig | \
     sed 's=\#[0-9][0-9]*=<eps>=g' | \
     fstcompile --isymbols=$wrddir/phones.txt --osymbols=$wrddir/words.txt \
  ...
  utils/make_lexicon_fst.pl --pron-probs $tmpdir/lexiconp.txt $sil_prob $silphone | \
    fstcompile --isymbols=$wrddir/phones.txt --osymbols=$wrddir/words.txt \
    --keep_isymbols=false --keep_osymbols=false | \
     fstarcsort --sort_type=olabel > $wrddir/L.fst || exit 1;
fi
# The file oov.txt contains a word that we will map any OOVs to during
# training.
echo "$oov_word" > $wrddir/oov.txt || exit 1;
cat $wrddir/oov.txt | utils/sym2int.pl $wrddir/words.txt >$wrddir/oov.int || exit 1;
...
echo '#0' >$wrddir/phones/wdisambig.txt
utils/sym2int.pl $wrddir/phones.txt <$wrddir/phones/wdisambig.txt >$wrddir/phones/wdisambig_phones.int
utils/sym2int.pl $wrddir/words.txt <$wrddir/phones/wdisambig.txt >$wrddir/phones/wdisambig_words.int
...
for f in silence nonsilence optional_silence disambig context_indep; do
  utils/sym2int.pl $wrddir/phones.txt <$wrddir/phones/$f.txt >$wrddir/phones/$f.int
  utils/sym2int.pl $wrddir/phones.txt <$wrddir/phones/$f.txt | \
   awk '{printf(":%d", $1);} END{printf "\n"}' | sed s/:// > $wrddir/phones/$f.csl || exit 1;
done
for x in sets extra_questions; do
  utils/sym2int.pl $wrddir/phones.txt <$wrddir/phones/$x.txt > $wrddir/phones/$x.int || exit 1;
done
utils/sym2int.pl -f 3- $wrddir/phones.txt <$wrddir/phones/roots.txt \
   > $wrddir/phones/roots.int || exit 1;
#if $position_dependent_phones; then
if [ -f $wrddir/phones/word_boundary.txt ]; then
  utils/sym2int.pl -f 1 $wrddir/phones.txt <$wrddir/phones/word_boundary.txt \
    > $wrddir/phones/word_boundary.int || exit 1;
fi
silphonelist=`cat $wrddir/phones/silence.csl`
nonsilphonelist=`cat $wrddir/phones/nonsilence.csl`
utils/gen_topo.pl $num_nonsil_states $num_sil_states $nonsilphonelist $silphonelist >$wrddir/topo
...
if $silprob; then
  utils/make_lexicon_fst_silprob.pl $tmpdir/lexiconp_silprob_disambig.txt $srcdir/silprob.txt $silphone '#'$ndisambig | \
     fstcompile --isymbols=$wrddir/phones.txt --osymbols=$wrddir/words.txt \
     --keep_isymbols=false --keep_osymbols=false |   \
     fstaddselfloops  $wrddir/phones/wdisambig_phones.int $wrddir/phones/wdisambig_words.int | \
     fstarcsort --sort_type=olabel > $wrddir/L_disambig.fst || exit 1;
else
...
! utils/validate_lang.pl $wrddir && echo "$(basename $0): error validating output" &&  exit 1;
( echo -e "<eps>\t0"; echo -e "<s>\t1"; echo -e "</s>\t2"; echo -e "PSI1\t3"; echo -e "<PHNSILSP>\t4"; \
  cat $srcdir/nonsilence_phones.txt | awk '{print"PHN_"$1"\t"NR+4}' ) > $phndir/words.txt
#  | cat - $dir/lexicon1.txt $dir/MSU_single_letter.txt  > $dir/lexicon2.txt || exit 1;
cat $srcdir/nonsilence_phones.txt | awk '{print"PHN_"$1" "$1}' > $phndir/phn.dict
exit 0;
