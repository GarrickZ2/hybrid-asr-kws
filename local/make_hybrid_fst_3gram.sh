...
./local/format_lm_sri_phn.sh --srilm-opts "-subset -prune-lowprobs -order 3" $phn_dir $phn_dir/phn.3gram.lm $phn_dir/phn.dict $phn_graph_dir
...
./utils/format_lm_sri.sh --srilm-opts "-subset -prune-lowprobs -unk -map-unk <UNK> -order 3" $wrd_dir data/local/local_lm/data/hybrid/word.3gram.lm $wrd_dir/lexicon.txt $wrd_graph_dir
...
./local/make_PHNloop_FST_uppercase.sh $phn_graph_dir/G_PHN_psi.txt $wrd_graph_dir/words.txt ${UNK_PENALTY} > $phn_graph_dir/G_PHNm.txt
